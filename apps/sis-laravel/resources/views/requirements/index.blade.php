...
<div class="timeline-tabs"><a class="{{ $tab === 'initial' ? 'active' : '' }}" href="{{ route('requirements.index', request()->except('tab') + ['tab' => 'initial']) }}">Initial payout</a><a class="{{ $tab === 'renewal' ? 'active' : '' }}" href="{{ route('requirements.index', request()->except('tab') + ['tab' => 'renewal']) }}">Renewal payout</a></div>
...
@php($progress = $studentCycle->requirementProgress())
<tr><td>{{ $studentCycle->student->full_name }}</td><td>{{ $studentCycle->student->student_id }}</td><td>{{ $studentCycle->school ?: '—' }}</td><td>{{ $progress['complete'] }}/{{ $progress['total'] }}</td><td>{{ ucfirst($studentCycle->qualification_status) }}</td><td><a href="{{ route('requirements.edit', [$studentCycle, 'tab' => $tab]) }}">Manage requirements</a></td></tr>
