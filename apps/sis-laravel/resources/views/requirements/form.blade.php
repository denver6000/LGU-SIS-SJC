@extends('layouts.app')
@section('content')
<div class="page-heading"><div><p class="eyebrow">Requirements Timeline · {{ $studentCycle->academicCycle->label() }}</p><h1>{{ $studentCycle->student->full_name }}</h1><p class="muted">Choose the student’s payout type, then manage only the requirements relevant to that type.</p></div><a href="{{ route('requirements.history', [$studentCycle->student, 'tab' => 'requirements', 'cycle_id' => $studentCycle->academic_cycle_id]) }}">View student history</a></div>
...
<div class="form-section requirement-panel" data-requirement-panel="initial"><div class="dialog-history-header"><h2>Initial payout requirements</h2><button type="button" class="secondary-button compact" data-select-requirements="initial">Select all</button></div><div class="requirement-grid">@foreach($initialFields as $field => $label)<label class="check"><input type="checkbox" name="{{ $field }}" value="1" @checked($studentCycle->requirements?->{$field}) data-requirement-checkbox="initial"> {{ $label }}</label>@endforeach</div></div>
<div class="form-section requirement-panel" data-requirement-panel="renewal"><div class="dialog-history-header"><h2>Renewal payout requirements</h2><button type="button" class="secondary-button compact" data-select-requirements="renewal">Select all</button></div><div class="requirement-grid">@foreach($renewalFields as $field => $label)<label class="check"><input type="checkbox" name="{{ $field }}" value="1" @checked($studentCycle->requirements?->{$field}) data-requirement-checkbox="renewal"> {{ $label }}</label>@endforeach</div></div>
...
document.querySelectorAll('[data-select-requirements]').forEach((button) => button.addEventListener('click', () => {
    const type = button.dataset.selectRequirements;
    const checkboxes = [...document.querySelectorAll(`[data-requirement-checkbox="${type}"]`)].filter((checkbox) => !checkbox.closest('[hidden]'));
...
document.querySelectorAll('[data-requirement-panel]').forEach((panel) => {
    const type = panel.dataset.requirementPanel;
    const button = document.querySelector(`[data-select-requirements="${type}"]`);
    const checkboxes = [...panel.querySelectorAll('[data-requirement-checkbox]')];
    if (button && checkboxes.length && checkboxes.every((checkbox) => checkbox.checked)) button.textContent = 'Deselect all';
});
</script>
@endsection
