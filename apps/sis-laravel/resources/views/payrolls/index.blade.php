...
<div class="payroll-summary"><span><strong>{{ $payrollRows->count() }}</strong> {{ $status === 'payrolled' ? 'payrolled' : 'ready for payroll' }}</span><span><strong>{{ $qualifiedCycles->count() }}</strong> manually qualified</span></div>
<div class="form-grid payroll-metadata-grid">@if($status === 'ready')<label>Date Of Filing<input type="date" name="date_of_filing"></label><div><button class="primary-button" type="button" data-payroll-export>Create Payroll Files</button></div>@endif</div>
<div class="table-card"><table><thead><tr><th>{{ $status === 'ready' ? 'Select' : 'Status' }}</th><th>Student</th><th>Student ID</th><th>Payout type</th><th>School</th><th>Requirements</th><th>Payroll state</th></tr></thead><tbody>
