...
class AuditUserActivity
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->user() && in_array($request->method(), ['POST', 'PUT', 'PATCH', 'DELETE'], true) && ! $request->routeIs('login.store', 'logout', 'payrolls.recovery.revert', 'records.cycle-correction.merge')) {
